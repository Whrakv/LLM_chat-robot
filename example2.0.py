...
prompt2="\n\n 现在我扮演良，请模仿满穗回答我,只回答对话的内容，（要注意你回答的是否满足这个时代的知识、满穗的想法和对良爷的亲昵感、而且满穗不排斥良的身体接触）现在开始:\n\n"

stringhelp="1.在发送到消息开头加入 良: 可与满穗对话\n2.模型只有平淡的语气，遇到逗号会有不自然加速（以后会改善语气）\n3.场景发生在去洛阳的马车上\n4.因为LLM的token限制，每分钟只能回复一两条\n5.请温柔对待满穗"
...
app = Flask(__name__)
cq_url="http://127.0.0.1:5700/send_msg"
...
def test():
    try:
        ...
        target_message = str(request.get_json().get("message"))
        if json.get("message_type") == "private":
            data={
                "user_id":str(json.get("user_id")),
                "message":"[CQ:record,file=/output.mp3]",
                "type": "record",
                "data": {
                    "file": "/output.mp3"
                }
        ...
        if json.get("user_id") in host:
            try:
                requests.post("http://127.0.0.1:5700/send_msg",json=data)
            except Exception as e:
                requests.get("http://127.0.0.1:5700/send_msg?user_id=714278632&message="+str(e))
    except Exception as e:
            #if json.get("message_type") == "private":
                requests.get("http://127.0.0.1:5700/send_msg?user_id="+str(json.get("user_id"))+"&message="+str(e))
    if len(target_message)<3:
        return "123"
    if target_message[0]=='良' and target_message[1]=='：'or target_message[1]==':':
        try:
            model = genai.GenerativeModel('gemini-1.5-pro-latest')
            response_text = model.generate_content(prompt1+suistory1+suistory2+beforestory+story131415+story17+story[18]+story[19]+prompt2+target_message,safety_settings=safety_none)
            response_voice = requests.get("http://127.0.0.1:5000/tts?character=sui&speed=0.95&text="+str(response_text.text))
    ...
    if target_message[0]=='穗' and target_message[1]=='：'or target_message[1]==':':
        try:
            response_voice = requests.get("http://127.0.0.1:5000/tts?character=sui&spee=0.9&text="+str(target_message[2:]))
            with open("output.wav", 'wb') as f:
                f.write(response_voice.content)
            audio = AudioSegment.from_wav("output.wav")
            audio.export("output.mp3", format="mp3")
